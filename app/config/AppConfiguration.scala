...
class AppConfiguration(configuration: Configuration) {
  ...
  /**
    * Please note that if you want to have the system fail if configuration is not available,
    * use `loadOrThrow` .
    * If you want the system to "recover", use `load` that returns a `SyncResult`
    */

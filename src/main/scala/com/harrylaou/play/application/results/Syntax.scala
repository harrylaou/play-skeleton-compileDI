...
object Syntax extends EitherSyntax {
  ...
  implicit class RichAsyncResultT[A](val asyncResultT: AsyncResultT[A]) extends AnyVal {
    ...
    /** to run a side-effect like logging and return the same asyncResult is like `fut.andThen {case Success(x) =>
      * ....}`
      */
    ...
    def recoverWithDefault(
      ...
      asyncResultT.recover { case error =>
        logger.error(error)
        default
      }
  ...
  def cleanAppErrors[A](list: List[SyncResult[A]])(implicit logger: AppLogger): List[A] = {
    val tup: (List[SyncResult[A]], List[SyncResult[A]]) = list.partition(_.isLeft)
    val errors: List[SyncResult[A]]                     = tup._1
    val models: List[SyncResult[A]]                     = tup._2
    errors.foreach(e => logger.error(e.swap.toOption.get))
  ...
  implicit class RichFutureTry[A](val futureTryA: Future[Try[A]]) extends AnyVal {
    ...
    def liftAsync(
      ...
      AsyncResultT[A](
        futureTryA.map(_.liftSync(errStatus)).recover { case ex: Exception =>
          Left(AppError.fromS(Results.BadGateway)(ex.getMessage))
        }
      )
  }
